...
| _Response_ | An optional StationXML block that can be used to describe a response stage applied after the sensor stages and prior to the datalogger stages. This usually describes a pre-amp, scale factor and bias values if given will be assumed to apply to the sensitivity or polynomial of the sensor as well as the given response stage.

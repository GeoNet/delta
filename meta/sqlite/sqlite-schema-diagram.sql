...
SELECT
    ...
    END || '
                <TR>
                    ...
                    <TD>' || i.name || '</TD>
                    <TD>' ||
                        CASE i.type WHEN 'TEXT' THEN 'ⓐ' WHEN 'INTEGER' THEN 'ℕ' WHEN 'REAL' THEN 'ℝ' WHEN 'BOOLEAN' THEN '☑' WHEN 'DATETIME' THEN '⌚' ELSE i.type END || 
                    '</TD>
                    <TD PORT="' || i.name || '_from">' ||
                       CASE i."notnull" WHEN 0 THEN '' ELSE '☀' END ||
                    '</TD>
                </TR>
            ' ||
    CASE

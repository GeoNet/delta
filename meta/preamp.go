...
// Preamps returns a sorted slice of single Preamp entries.
func (g Preamp) Preamps() []Preamp {
	...
	sort.Slice(preamps, func(i, j int) bool { return preamps[i].Less(preamps[j]) })
...
func (g *PreampList) decode(data [][]string) error {
	...
	for _, d := range data[1:] {
		...
		gain, err := strconv.ParseFloat(d[preampGain], 64)
		if err != nil {
			return err
		}

...
func EquipmentType(e *stationxml.EquipmentType) string {
	/*
		return fmt.Sprintf("ResourceId: %q, Type: %q, Description: %q, Manufacturer: %q, Vendor: %q Model: %q, SerialNumber: %q, InstallationDate: %q, RemovalDate: %q",
			e.ResourceId, e.Type, e.Description, e.Manufacturer, e.Vendor, e.Model, e.SerialNumber, e.InstallationDate.String(), e.RemovalDate.String(),
		)
	*/
	return e.Description
}

func (d Decoder11) Decode(verbose bool, data []byte) ([]byte, error) {
		...
		for s := range net.Station {
			...
			for c := range root.Network[n].Station[s].Channel {
				...
				if cha.DataLogger != nil {
					fmt.Printf("datalogger %s\n", EquipmentType(cha.DataLogger))
					cha.DataLogger = nil
				}
				if cha.Sensor != nil {
					fmt.Printf("sensor:    %s\n", EquipmentType(cha.Sensor))
					cha.Sensor = nil
				}

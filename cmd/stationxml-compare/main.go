...
func decode(version string, verbose bool, path string) ([]byte, error) {
...
func main() {
	...
	switch n := flag.NArg(); {
	case n > 1:
		first, err := decode(version, false, flag.Args()[0])
		if err != nil {
			log.Fatal(err)
		}

		second, err := decode(version, false, flag.Args()[1])
		if err != nil {
			log.Fatal(err)
		}
	...
	case n > 0:
		if _, err := decode(version, true, flag.Args()[0]); err != nil {
			log.Fatal(err)
		}
	default:

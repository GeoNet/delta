...
func TestStreams(t *testing.T) {
	...
	t.Run("check stream overlaps", func(t *testing.T) {
	...
	/*
		...
		stasites := make(map[string]map[string]meta.Site)
		for _, s := range sites {
			if _, ok := stasites[s.Station]; !ok {
				stasites[s.Station] = make(map[string]meta.Site)
			}
			stasites[s.Station][s.Location] = s
		}
		...
		for _, c := range streams {
			...
			} else if s, ok := stasites[c.Station]; !ok {
				t.Log("unknown stream station: " + c.Station)
